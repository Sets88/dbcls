...
| `.VARS` | Open `_vars` as a **blocking, editable** `key`/`value` sheet (like `.VIEW`; opens even when empty), then return them as `[{key, value}, ...]` rebuilt after it closes. Edits hit the store immediately and are visible to the following steps: `e` sets key (renames) / value (as a string), `z=`/`g=` set the value to a Python expression, `a` adds, `d`/`gd` delete, `U` undoes. Usable standalone, mid-pipeline or as last step. |
| `.SHEET NAME` | Create a VisiData sheet named `NAME` (template) from the current rows, pass data through unchanged. Built in the background as the step runs (never blocks, survives a cancelled run; reachable mid-run via VisiData's `Shift+S` from a picker sheet, via `Alt+S` afterwards); the whole stack opens when the pipeline finishes. Inside `.WHILE` that is one sheet per iteration — name them apart, e.g. `.SHEET "articles {{_i}}"`. |
| `.VIEW NAME` | Blocking `.SHEET`: shows the rows immediately and waits until the user closes them with `q`, then passes data through unchanged. This is what you want inside a `.WHILE` loop or a `.FN` function. Closing it is not an answer — it never cancels the pipeline. As the **last** step (like `.VARS`) the rows are not opened a second time. |
...
- The condition's **value** (marked rows, next page, …) is the input of the body's first step
  and is exposed as `_i` / `{{_i}}`.
...
- `br()` ends the loop with that iteration's data, `stop()` aborts the pipeline, Esc cancels it.
...
- `info(msg)` / `warn(msg)` — non-blocking / blocking popup. `warn()`'s Esc cancels the whole
  pipeline with **no result shown**; `info()`'s Esc requests a stop at the next step boundary.
...
- `set_var(name, val)` / `get_var(name, default=None)` — same store as `.SET_VAR`/`.GET_VAR`
  and the editor's `gT` (save row(s)) / `gzT` (save column values) keybindings.
- Row prompts come as two pairs — popup over the editor vs. sheet in VisiData, s-less name picks
  ...
  pipeline continues with. For a menu use `choose()` or `schoose()`: comparing a plural result
...
- `input(title, default=None, items=None)`, `ask(title)` — text / yes-no prompts. `input()`
  recalls earlier answers to the same title with `↑`/`↓` (what is typed filters them); `items`
  seeds that list with strings or rows of a previous step, e.g. `input('path', items=data)`.
  Its Esc first closes that list, and only cancels the pipeline when no list is up. `ask()` is
  strict: `y`/Enter → True, `n` → False, Esc cancels, **any other key is ignored** and the
  question keeps waiting.
- **Esc (q in the VisiData sheets) always cancels the whole pipeline with no result** — different
...
9. A `.WHILE` condition that can never turn falsy (`"True"`, or a list that is always non-empty
   like `[0]`)? → the loop only ends on Esc, `br()`, `stop()` or the 100000-iteration guard.
